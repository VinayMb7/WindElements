# Testimonial

Customer testimonial card with rating.

## Installation

```bash
npx windelements@latest add testimonial
```

## Usage

### Basic Example

```typescript
import { createTestimonial } from './components/ui/testimonial';

const component = createTestimonial({
  // Component props
});

document.body.appendChild(component.getElement());
```

### With Custom Styling

```typescript
const component = createTestimonial({
  className: 'custom-class hover:scale-105',
  // Other props
});
```

## Props

See the TypeScript interface in the component source file for all available props and options.

```typescript
import { TestimonialProps } from './components/ui/testimonial';
```

## Examples

Check the [Examples](/examples) page for real-world usage patterns.

## API Reference

### Factory Function

```typescript
const component = createTestimonial(props);
const element = component.getElement();
```

### Class-based API

```typescript
import { Testimonial } from './components/ui/testimonial';

const component = new Testimonial(props);
const element = component.getElement();
```

## Accessibility

This component follows WAI-ARIA best practices for accessibility.

## Related Components

Browse all [Components](/components/) to see related options.

## Styling

Customize the component using the `className` prop and CSS custom properties:

```typescript
const component = createTestimonial({
  className: 'bg-primary text-primary-foreground rounded-lg'
});
```
